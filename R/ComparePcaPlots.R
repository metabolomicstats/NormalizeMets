...
#' @param n The number of principal components to be plotted. The default value is set to 3.
...
ComparePcaPlots <- function(lfeaturedata, lgroupdata, saveplot=FALSE, 
                     ...
                     lmain=NULL, n=3, cols=NULL,cex_val = 0.7, ...)
{
  ...
  if (multiplot) {
    ...
    if (num_of_plots == 2){
      ...
      print(pairgg[[2]], vp = viewport(layout.pos.row = 1, layout.pos.col = c(5:7)))
    } else {
      grid.newpage()
      ...
      if (num_of_plots == 4){ 
        print(pairgg[[4]], vp = viewport(layout.pos.row = 2, layout.pos.col = c(5:7)))
      }
    }
